...
import { applyFinishResponse } from './stats.js';
...
export async function startSession(deckId, mode) {
  ...
  const onCorrect = (cardId, opts = {}) => {
    ...
    // Eventos
    emit('correct', { deckId, cardId, mode, combo, timeMs: opts.timeMs });
    for (const m of COMBO_SOUND_MILESTONES) {
      if (combo === m) {
        emit('comboMilestone', { combo: m });
        lastMilestoneFired = m;
        break;
      }
    }
  ...
  const onWrong = (cardId) => {
    ...
    pendingFlush.push(ev);
    playSfx('wrong');
    emit('wrong', { deckId, cardId, mode });
    if (hadCombo >= 5) emit('comboBreak', { from: hadCombo });
  };
